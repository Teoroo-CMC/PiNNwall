...
def make_Dfield_eta(eta, coord, cell):
    """ Compute the softness kernel at finite D from the one at finite E

    Args:
        eta (tensor): a [B, N, N] tensor containing the hardness kernel
        coord (tensor) : a [B, N, N] tensor containing the atomic positions
        cell (tensor) : a [B, 3, 3] tensor containing the cell parameters
...
def make_diag(atom_rind, params, nbatch, nmax):
    """ Construct a diagonal matriix with atomistic predictions"""
